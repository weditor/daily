...
上面说的 _接口代理_ 是对整个类进行代理，而属性代理是对单个成员进行代理。

还是这个例子, 我们想要监听属性地 get、set；使用一般的代码我们可以这么实现:
...
可以看到, 我们对 name、email 的监听代码其实非常类似，我们有没有什么办法复用呢？属性代理就派上用场了!

首先我们要实现一个代理类，代理类有两个特殊的方法: getValue/setValue;
